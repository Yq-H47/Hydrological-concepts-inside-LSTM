...
class DataforIndividual():
    ...
    #\camels_data\basin_mean_forcing\daymet\01
    ...
    #\camels_data\usgs_streamflow\01
    def load_flow_data(self, working_path, huc_id, basin_id, area):
        ...
        flow_data['flow(mm)'] = 28316846.592 * flow_data['Q'] * 86400 / (area * 10 ** 6)  # （feet/m2 to mm/s) -> mm/day  ->
        return flow_data
    ...
    def load_data(self):
        ...
        pd_data = merged_data[['prcp(mm/day)', 'tmin(C)', 'tmax(C)', 'srad(W/m2)', 'vp(Pa)', 'flow(mm)']]
        return pd_data
