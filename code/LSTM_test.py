...
from libs_tf import ScaleLayer, LSTM_tsetQ
...
attrs_path = 'your_path/s_391attrs.csv'

testing_start = '2005-10-01'
...
def create_model(input_xd_shape, seed):
    ...
    hn = LSTM_tsetQ(input_xd=32, hidden_size=64, seed=seed)(xd_input_forprnn)
...
def mse_metrics(y_true, y_pred):
    ...
    numerator = K.mean(K.square(y_true - y_pred), axis=1)
    #denominator = K.sum(K.square(y_true - K.mean(y_true, axis=1, keepdims=True)), axis=1)
    mse = numerator
...
def kge_metrics(y_true, y_pred):
    ...
    r = np.correlate(y_pred,y_true)

    std_pred = K.std(y_pred, axis=-2, keepdims=True)
    std_obs = K.std(y_true, axis=-2, keepdims=True)

    mean_pred = K.mean(y_pred, axis=-2, keepdims=True)
    mean_obs = K.mean(y_true, axis=-2, keepdims=True)

    numerator = K.sqrt(K.square(r - 1) + K.square(std_pred/std_obs - 1) + K.square(mean_pred/mean_obs - 1), axis=1)
...
def generate_train_test(test_set):
    ...
    test_x_np[:, 0:1] = (test_x_np[:, 0:1] - 3.2101783551512004) / 7.762683956382427
    test_x_np[:, 1:2] = (test_x_np[:, 1:2] - 5.961311828122113) / 10.505841525621062
    test_x_np[:, 2:3] = (test_x_np[:, 2:3] - 18.224939218048405) / 11.1392195803781
    test_x_np[:, 3:4] = (test_x_np[:, 3:4] - 333.6702087451168) / 115.5954085038271
    test_x_np[:, 4:5] = (test_x_np[:, 4:5] - 1118.8605627962972) / 703.1587205464945

    test_y_np = test_set_.values[:, -1:]
    test_y_np_nor = (test_y_np[:, -1:] - 1.1788026357318797) / 2.6435151845295635
...
nse_results = pd.DataFrame(columns=['Basin_ID', 'NSE_TEST', 'MSE_TEST'])



Path(f'{working_path}/results').mkdir(parents=True, exist_ok=True)
save_path_ealstm = f'model_path/model.h5'
model = create_model((1826, 32), seed=200)


for j in range(len(testx_list)):

    flow = stest_model(model=model, test_xd=testx_list[j], save_path=save_path_ealstm)
    nse_test = nse_metrics(testy_list[j], flow)
    mse_test = mse_metrics(testy_list[j], flow)
    #kge_test = kge_metrics(testy_list[j], flow)
    nse_results = nse_results.append({'Basin_ID': basin_id[j],
                                      'NSE_TEST': np.squeeze(K.eval(nse_test)), 'MSE_TEST': np.squeeze(K.eval(mse_test))},
                                     ignore_index=True)

    print(f"{basin_id[j]}_nse_mse:", K.eval(nse_test), K.eval(mse_test))

nse_results.to_csv(f'reslut_path/result_.csv', index=False)
